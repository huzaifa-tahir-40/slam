...
class Logger(Node):
    def __init__(self):
        ...
        self.u_att_received = False
        self.u_rep_received = False
        self.error_received = False
        self.cmd_vel_received = False
        self.robot_received = False

        self.u_att_sub = self.create_subscription(
            Vector3,
            '/U_att',
            self.u_att_callback,
            10
        )
        self.u_rep_sub = self.create_subscription(
            Vector3,
            '/U_rep',
            self.u_rep_callback,
            10
        ...
        logs_dir = os.path.expanduser('~/apf_ws/my_logs/apf_pkg_logs')
        os.makedirs(logs_dir, exist_ok=True)
        timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = f'apf_pkg_log_{timestamp}.csv'
    ...
    def u_att_callback(self, msg):
        if not self.u_att_received:
            self.get_logger().info('DEBUG: first /U_att message received')
        self.u_att = msg
        self.u_att_received = True

    def u_rep_callback(self, msg):
        if not self.u_rep_received:
            self.get_logger().info('DEBUG: first /U_rep message received')
        self.u_rep = msg
        self.u_rep_received = True

    def error_callback(self, msg):
        if not self.error_received:
            self.get_logger().info('DEBUG: first /error_msg message received')
        self.error = msg
    ...
    def log_data(self):
        if not (
            self.u_att_received and
            self.u_rep_received and
            self.error_received and
            self.cmd_vel_received and
            self.robot_received
        ):
            self.get_logger().info(
                f'DEBUG: waiting on -> '
                f'U_att={self.u_att_received}, '
                f'U_rep={self.u_rep_received}, '
                f'error={self.error_received}, '
        ...
        self.writer.writerow([
            time,
            self.u_att.x, self.u_att.y,
            self.u_rep.x, self.u_rep.y,
            self.error.x, self.error.y,
